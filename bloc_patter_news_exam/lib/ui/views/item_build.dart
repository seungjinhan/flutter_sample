...
import '../../ui/screens/web_view.dart';
...
buildListSliver(NewsModel values, BuildContext context) {
  if (values.articles.length == 0) {
    return SliverToBoxAdapter(
      ...
      child: Center(child: Text('You didn\'t like anything', style: TextStyle(color: Theme.of(context).accentColor, fontSize: 24))),
    ));
  } else {
    return SliverList(
      delegate: SliverChildBuilderDelegate((context, index) => ListItemBuild(values.articles[index]), childCount: values.articles.length),
    );
  }
}
...
class ListItemState extends State<ListItemBuild> with TickerProviderStateMixin {
            ...
            child: ClipRRect(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(colorFilter: ColorFilter.mode(Colors.black54.withOpacity(colorAnimation.value), BlendMode.hardLight), fit: BoxFit.cover, image: image),
                ),
  ...
  _dateItemBuild() {
    ...
    return Container(
      ...
      child: Text(
        (int.tryParse(difference.inHours.abs().toString()) < 12) ? difference.inHours.abs().toString() + " hours ago" : difference.inDays.abs().toString() + " days ago",
  ...
  _openWebSite(Articles model, context) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => WebViewScreen(model: model)));
  }
  ...
  _textItemBuild() {
    return Flexible(
      child: new Text(
        ...
        style: new TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white),
      ),
    );
  }
}
