...
class SettingsState extends State<SettingsScreen> {
    ...
    return SafeArea(
      child: SingleChildScrollView(
        ...
        child: Column(
          children: <Widget>[
            Container(
              decoration: BoxDecoration(border: Border.all(color: Theme.of(context).accentColor, width: 2), borderRadius: BorderRadius.circular(16)),
              margin: EdgeInsets.only(bottom: 10, left: 10.0, right: 10.0, top: 10.0),
            ...
            ListTile(
              ...
              trailing: Switch(
                ...
                value: Theme.of(context).brightness == Brightness.dark ? swTheme = true : swTheme = false,
              ),
            ),
            Divider(), // Divider
            ListTile(
  ...
  changeTheme(value) async {
    SharedPreferences prefs = await _prefs;
    DynamicTheme.of(context).setThemeData(ThemeData(accentColor: Theme.of(context).accentColor, brightness: Theme.of(context).brightness == Brightness.dark ? Brightness.light : Brightness.dark));
    setState(() {
      swTheme = value;
    });
  ...
  showPickerArray(BuildContext context) async {
    ...
    Picker(
        ...
        adapter: PickerDataAdapter<String>(pickerdata: JsonDecoder().convert(PickerCountry), isArray: true),
        hideHeader: true,
        title: new Text("Please Select"),
        onConfirm: (Picker picker, List value) {
          prefs.setString('country', picker.getSelectedValues().toString().replaceAll('[', '').replaceAll(']', ''));
        }).showDialog(context);
  }
          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                ...
                FlatButton(
                    onPressed: () async {
                      DynamicTheme.of(context).setThemeData(ThemeData(accentColor: local, brightness: Theme.of(context).brightness));
