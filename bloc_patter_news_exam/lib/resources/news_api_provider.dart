...
class NewsApiProvider {
  ...
  Future<NewsModel> getFavoriteNews() async {
    ...
    if (articles.data != null) {
      for (int i = 0; i < articles.data.length; i++) list.add(Articles.fromJson(articles.data.values.toList()[i]));
    }
    nm.articles = list;
    return nm;
  }
  ...
  addToFiresstore(val) async {
    ...
    Firestore.instance.collection('users').document(await getMyUID()).setData({key: val}, merge: true);
  }

  deliteFromFirestore(val) async {
    final String key = val['url'].toString().replaceAll('/', '').replaceAll('.', '');
    Firestore.instance.collection('users').document(await getMyUID()).updateData({key: FieldValue.delete()});
  }
}
